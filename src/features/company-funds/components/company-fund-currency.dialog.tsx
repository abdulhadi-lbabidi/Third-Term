...
import type { CompanyFundCurrency } from '../types';
import { attachFundCurrencySchema, type AttachFundCurrencyValues } from '@/features/funds/schemas/funds.schema';

type CompanyFundCurrencyDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  currency?: CompanyFundCurrency | null;
  onSubmit: (values: { currency_id: number; balance: string }) => Promise<void>;
  loading?: boolean;
};

export function CompanyFundCurrencyDialog({
  open,
  onOpenChange,
  currency,
  onSubmit,
  loading,
}: CompanyFundCurrencyDialogProps) {
  const form = useForm<AttachFundCurrencyValues>({
    resolver: zodResolver(attachFundCurrencySchema) as any,
