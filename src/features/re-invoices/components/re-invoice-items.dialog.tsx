...
export function ReInvoiceItemsDialog({ id, onClose }: { id?: number; onClose: () => void }) {
  ...
  const save = useMutation({ mutationFn: () => { const payload = { reinvoice_id: id, material_id: materialId, unit, quantity, unit_price: price, item_description: description }; return editingItem ? reInvoicesApi.updateItem(editingItem.id, payload) : reInvoicesApi.createItem(payload); }, onSuccess: async () => { await client.invalidateQueries({ queryKey: ['re-invoice-items', id] }); reset(); } });
