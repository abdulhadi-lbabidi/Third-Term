...
export function useSaveReInvoice() {
  const client = useQueryClient();
  return useMutation({
    mutationFn: ({ id, payload }: { id?: number; payload: ReInvoicePayload }) => id ? reInvoicesApi.update(id, payload) : reInvoicesApi.create(payload),
    onSuccess: async (_, variables) => {
      await client.invalidateQueries({ queryKey: RE_INVOICES_KEY, refetchType: 'all' });
      await client.invalidateQueries({ queryKey: ['projects'], refetchType: 'active' });
      await client.invalidateQueries({ queryKey: ['project-funds'] });
      await client.invalidateQueries({ queryKey: ['company-funds'] });
      await client.invalidateQueries({ queryKey: ['funds'] });
      if (!variables.id) {
        const rawProjId = (variables.payload as any)?.project_id;
        if (rawProjId && Number.isFinite(Number(rawProjId)) && Number(rawProjId) > 0) {
          ...
          await client.invalidateQueries({ queryKey: ['projects', Number(rawProjId)] });
          await client.invalidateQueries({ queryKey: ['projects'] });
        }
      }
      toast.success(variables.id ? 'تم تحديث المرتجع' : 'تم إنشاء المرتجع');
    },
  });
}
