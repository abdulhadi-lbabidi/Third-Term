...
export const reInvoicesApi = {
  ...
  create: async (payload: ReInvoicePayload) => (await apiClient.post('/re-invoices', payload)).data?.data,
  update: async (id: number, payload: Partial<ReInvoicePayload>) => (await apiClient.patch(`/re-invoices/${id}`, payload)).data?.data,
