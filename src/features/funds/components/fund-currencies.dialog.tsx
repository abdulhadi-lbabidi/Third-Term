...
import type { Fund, FundCurrency } from '../types';

type FundCurrenciesDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  fund: Fund | null;
  onCurrencyClick?: (fund: Fund, currencyId: number) => void;
};

export function FundCurrenciesDialog({ open, onOpenChange, fund, onCurrencyClick }: FundCurrenciesDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[640px]">
        <DialogHeader>
          <DialogTitle>عملات {fund?.name || 'الصندوق'}</DialogTitle>
        </DialogHeader>

        <div className="space-y-3">
          {currencies.length ? (
            currencies.map((currency: FundCurrency) => (
              <button
                key={currency.id}
                type="button"
